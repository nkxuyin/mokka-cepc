 ...
 *
$Id: SLcal_cepc.cc 23 2009-12-17 11:35:56Z bogdan $
 * SLcal01.cc - v1.0 Nov. 2006
 * SLcal02.cc - v1.1 Oct. 2008
 * SLcal_cepc.cc - v1.3 Oct. 2009
 *
 * Version history:
...
INSTANTIATE(SLcal_cepc)


SLcal_cepc::SLcal_cepc() : VSuperSubDetectorDriver("SLcal_cepc")
{

}

SLcal_cepc::~SLcal_cepc()
{
}

G4bool SLcal_cepc::PreLoadScriptAction(Database* dbtmp,
			     CGAGeometryEnvironment& theEnv)
{
  G4String query;

  G4cout << G4endl <<"SLcal_cepc - the LumiCal super driver v1.1" << G4endl;
  G4String dbName = theEnv.GetDBName() ;
  G4cout << " Database name : " << dbName << G4endl ;

//modify by xuyin: add parameter LStar_zbegin
...
////////// create the lumical table ////////////////
  ...
  query += "  LStar_zbegin double default NULL\n";
...
//preload env values to the db
  ...
  query += " @LStar_zbegin = " + theEnv.GetParameterAsString("LStar_zbegin") + ";";
  dbtmp->exec(query.data());


  
//  G4cout << "SLcal_cepc : " << query << G4endl;
...
//insert'em into the lumical table
  ...
  query += "@silicon_thickness, @layer_gap, @tile_gap, @crossing_angle,@LStar_zbegin);";
...
G4double LStar_zbegin = theEnv.GetParameterAsDouble("LStar_zbegin");
...
//  G4cout << "SLcal_cepc::PreLoadScriptAction() done." << G4endl;
  return true;
}


G4bool SLcal_cepc::PostLoadScriptAction(Database*  dbtmp,
			     CGAGeometryEnvironment& )
{
  ...
  G4cout << "SLcal_cepc has set the following values :" << G4endl 
