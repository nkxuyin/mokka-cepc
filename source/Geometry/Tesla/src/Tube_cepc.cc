...
// *********************************************************
//
// $Id: Tube_cepc.cc,v 1.5 2008/10/15 16:51:53 steve Exp $
// $Name: mokka-07-00 $
//
// History:
...
// - modified for fancier geometries as Tube_cepc: Adrian Vogel, 2006-04-20

#include "Tube_cepc.hh"
...
INSTANTIATE(Tube_cepc)

G4bool Tube_cepc::ContextualConstruct(const CGAGeometryEnvironment &env, G4LogicalVolume *worldLog)
{
  ...
  while (db->getTuple()) {
    ...
    if (offsetValue != 0) {
      G4cout
        << "Tube_cepc: Using " << globalName << " = "
        << currentValue / mm << " mm instead of "
        << assumedValue / mm << " mm" << G4endl;
      usingOffsets = true;
    }
  }
  if (usingOffsets) Control::Log("Tube_cepc: Be sure you know what you're doing!");
...
zEnd=lstar_zbegin-220;
    ...
    if (crossingAngle == 0 && crossType != kCenter) {
      Control::Log("Tube_cepc: You are trying to build a crossing geometry without a crossing angle.\n"
    ...
    switch (crossType) {
      ...
      default: {
        Control::Log("Tube_cepc: Unimplemented \"crossType\" code.");
